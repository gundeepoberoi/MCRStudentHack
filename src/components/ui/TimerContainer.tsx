...
const TimerContainer = ({ currentDate }: { currentDate: Date }) => {
  ...
  let daysFlip =
    ...
    Number(seconds) <= 0
      ...
      : false;
  let hoursFlip =
    Number(seconds) <= 0 && Number(minutes) <= 0 && Number(hours) > 0
      ? true
      : false;
  ...
  return (
    <div className="rounded-xl">
      <div className="grid grid-cols-2 gap-4 px-10 md:flex md:items-center md:justify-between rounded-xl">
        <NumberBox num={days} unit="Days" flip={daysFlip} />
        <div className="hidden text-5xl -mt-8 md:inline-block md:text-7xl font-normal text-gray-50 ">
          :
        </div>
